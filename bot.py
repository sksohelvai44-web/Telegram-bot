...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_states and user_states[user_id].get('step') == 'waiting':
        ...
        if code:
            ...
            await update.message.reply_text("❌ Invalid 2FA Key! Please check and try again.", reply_markup=cancel_menu)
        return
    ...
    if text == "✅ Done":
        if user_id in user_states and user_states[user_id].get('step') == 'done':
            ...
            user_states.pop(user_id, None)
            await update.message.reply_text(
                "✅ Task Completed Successfully!\n\n💰 Reward: 4.00 BDT added to your balance!",
                reply_markup=main_menu
            )
        else:
            await update.message.reply_text("❌ No active task!", reply_markup=main_menu)
    ...
    if user_id in withdraw_states and withdraw_states[user_id].get('step') == 'amount':
        try:
            amount = float(text)
            if amount < 50:
                await update.message.reply_text(f"❌ Minimum 50.00 BDT!", reply_markup=cancel_menu)
                return
            if amount > db_user[3]:
                await update.message.reply_text(f"❌ Insufficient balance!", reply_markup=cancel_menu)
                return
